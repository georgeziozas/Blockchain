//We will use yarn to compile our code.
//RUN THIS TO COMPILE: yarn solcjs --bin --abi --include-path node_modules/ --base-path . -o . SimpleStorage.sol
// Ganache RPC: http://127.0.0.1:7545 , we are going to use a wrapper to interact with our node.

const ethers = require("ethers");
const fs = require("fs"); //We need this library in order to read from our .abi and .bin files
const { config } = require("process");
const { consumers } = require("stream");
require("dotenv").config();

async function main() {
  const provider = new ethers.providers.JsonRpcProvider(process.env.RPC_URL); //this line gives us a connection to the blockchain
  const encryptedJson = fs.readFileSync("./encryptedKey.json", "utf8");
  let wallet = new ethers.Wallet.fromEncryptedJsonSync(
    encryptedJson,
    process.env.PRIVATE_KEY_PASSWORD
  );
  wallet = await wallet.connect(provider);
  const abi = fs.readFileSync("./SimpleStorage_sol_SimpleStorage.abi", "utf-8");
  const binary = fs.readFileSync(
    "./SimpleStorage_sol_SimpleStorage.bin",
    "utf-8"
  );

  //object we use to deploy contracts| abi: so our code knows how to interact with the contract, binary:the main compiled code, wallet: sign and deploy
  const contractFactory = new ethers.ContractFactory(abi, binary, wallet);
  console.log(
    "##################################################\n Deploying, please wait... \n##################################################\n"
  );
  const contract = await contractFactory.deploy(); //transaction response is what you get when you create your transaction
  const transactionReceipt = await contract.deployTransaction.wait(1); //transactionReceipt is what you get when you wait for a block confirmation
  console.log(
    "##################################################\n Contract Object: \n##################################################\n ",
    contract
  );
  console.log(
    ` ################################################## \n Contract deployed to: \n##################################################\n ${contract.address}`
  );
  console.log(
    " ################################################## \n Here is the transaction: \n##################################################\n"
  );
  console.log(contract.deployTransaction);
  console.log(
    " ################################################## \n Here is the receipt: \n ################################################## \n "
  );
  console.log(transactionReceipt);

  //Get our favourite mumber from our contract
  const currentFavoriteNumber = await contract.retrieve();
  console.log(`Current Favorite Number: ${currentFavoriteNumber.toString()}`);
  //Call the store method of our contract
  const transactionResponse_store = await contract.store("7");
  const transactionReceipt_store = await transactionResponse_store.wait(1);
  console.log(
    " ################################################## \n Here is the receipt for Store Method: \n ################################################## \n",
    transactionReceipt_store
  );
  //Get our updated favourite number
  const updatedFavoriteNumber = await contract.retrieve();
  console.log(`Update Favorite Number is: ${updatedFavoriteNumber}`);

  /* Alternate Way to Send a Transaction - More Low Level 
    // const nonce = await wallet.getTransactionCount()
    // tx = {
    //   nonce: nonce,
    //   gasPrice: 100000000000,
    //   gasLimit: 1000000,
    //   to: null,
    //   value: 0,
    //   data: "",
    //   chainId: 1337,
    
    // Additionally, there is a v,r,and s variable that ethers handles for us.
    // This is the signature of the transaction.
    // There is a lot of math going on with those values, but that's how it's gaurenteed that the transaction is signed!
    // https://ethereum.stackexchange.com/questions/15766/what-does-v-r-s-in-eth-gettransactionbyhash-mean
    */
}

main()
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error);
    process.exit(1);
  });
